...
int main() {
	...
	/*--- Inicialización de variables ---*/
	Mat mediana = imread("Videos/esp32/esp32.jpg");
	VideoCapture Cap("Videos/esp32/esp32_v4_15fps.avi");
	VideoWriter writer = VideoWriter("Video1_esp32.avi/1024x768_15fps/1024x768_15fps.avi", VideoWriter::fourcc('X', 'V', 'I', 'D'), 15, Size(Cap.get(cv::CAP_PROP_FRAME_WIDTH), Cap.get(cv::CAP_PROP_FRAME_HEIGHT)));
	VideoWriter writer2 = VideoWriter("Video1_esp32.avi/1024x768_15fps/1024x768_15fps_SDF.avi", VideoWriter::fourcc('X', 'V', 'I', 'D'), 15, Size(Cap.get(cv::CAP_PROP_FRAME_WIDTH), Cap.get(cv::CAP_PROP_FRAME_HEIGHT)), false);
	/*--- Inicialización de variables ---*/
	ofstream archivo("Video1_esp32.avi/1024x768_15fps/TiemposProcesamiento.txt", fstream::out);
	ofstream Archivo2("Video1_esp32.avi/1024x768_15fps/PersonasContadas.txt", fstream::out);
	/*--- Inicialización de variables ---*/
	Mat Frame1, grayImage1, grayImage2, differenceImage, cannyImage;
	/*--- Inicialización de variables ---*/
	...
	int wi = 1024;
	int frameArea = he * wi;
	float areaTH = frameArea / static_cast<float>(64);
	float areaMAX = frameArea / static_cast<float>(2);
	int medio_arriba = int(2 * (he / 4));
	int medio_abajo = int(2 * (he / 4));
	int limite_arriba = int(0 * (he / static_cast<double>(4)));
	int limite_abajo = int(4 * (he / static_cast<double>(4)));
	int cont_arriba = 0;
	int cont_abajo = 0;
	float count_up = 0;
	float count_down = 0;
	...
	/*--- Inicialización de variables ---*/
	int personas_totales = 32;
	int personas_arriba = 18;
	int personas_abajo = 14;
	...
	while (1) {
		...
		/*morphologyEx(cannyImage, cannyImage, MORPH_OPEN, (getStructuringElement(MORPH_RECT, Size(3, 3))), Point(-1, -1), 1);
		morphologyEx(cannyImage, cannyImage, MORPH_CLOSE, (getStructuringElement(MORPH_RECT, Size(11, 11))), Point(-1, -1), 1);*/
		...
		imshow("mediana", cannyImage);
		writer.write(Frame1);
		writer2.write(cannyImage);
