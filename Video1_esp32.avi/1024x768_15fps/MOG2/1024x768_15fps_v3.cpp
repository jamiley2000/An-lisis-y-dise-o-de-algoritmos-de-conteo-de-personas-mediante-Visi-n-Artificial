...
public:
	...
	bool Arriba(int limite) {
		if (this->Tracks.size() >= 3) {
			if (!this->estado) {
				if ((this->Tracks[this->Tracks.size() - 1][1] < limite) && (this->Tracks[this->Tracks.size() - 3][1] > limite)) {
					this->estado = true;
...
int main() {
	...
	int frame = 1;
	/*--- Inicialización de variables ---*/
	Ptr<BackgroundSubtractorMOG2> subtractor = createBackgroundSubtractorMOG2(50, 50, true);
	/*--- Inicialización de variables ---*/
	...
	while (1) {
		...
		/*--------------------------- Preprocesamiento ---------------------------*/
		GaussianBlur(Frame1, Frame1, Size(7, 7), 0);
