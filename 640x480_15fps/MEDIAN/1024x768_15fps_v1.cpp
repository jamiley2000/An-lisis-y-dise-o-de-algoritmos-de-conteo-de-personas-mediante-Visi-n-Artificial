...
int main() {
	...
	/*--- Inicialización de variables ---*/
	Mat mediana = imread("Videos/TestVideo/TestVideo.jpg");
	VideoCapture Cap("Videos/TestVideo/TestVideo_15fps.avi");
	VideoWriter writer = VideoWriter("Video2_TestVideo.avi/640x480_15fps/MEDIAN/640x480_15fps.avi", VideoWriter::fourcc('X', 'V', 'I', 'D'), 15, Size(Cap.get(cv::CAP_PROP_FRAME_WIDTH), Cap.get(cv::CAP_PROP_FRAME_HEIGHT)));
	VideoWriter writer2 = VideoWriter("Video2_TestVideo.avi/640x480_15fps/MEDIAN/640x480_15fps_SDF.avi", VideoWriter::fourcc('X', 'V', 'I', 'D'), 15, Size(Cap.get(cv::CAP_PROP_FRAME_WIDTH), Cap.get(cv::CAP_PROP_FRAME_HEIGHT)), false);
	/*--- Inicialización de variables ---*/
	ofstream archivo("Video2_TestVideo.avi/640x480_15fps/MEDIAN/TiemposProcesamiento.txt", fstream::out);
	ofstream Archivo2("Video2_TestVideo.avi/640x480_15fps/MEDIAN/PersonasContadas.txt", fstream::out);
	/*--- Inicialización de variables ---*/
	Mat Frame1, Frame2, grayImage1, grayImage2, differenceImage, cannyImage;
	/*--- Inicialización de variables ---*/
	...
	while (1) {
		...
		cvtColor(mediana, grayImage2, COLOR_BGR2GRAY);
		
		//GaussianBlur(differenceImage, differenceImage, Size(5, 5), 0);
		...
		threshold(differenceImage, differenceImage, 60, 255, THRESH_BINARY);
		/*GaussianBlur(differenceImage, differenceImage, Size(9, 9), 0);
		threshold(differenceImage, differenceImage, 20, 255, THRESH_BINARY);*/
		/*Canny(differenceImage, cannyImage, 20, 75);
		dilate(cannyImage, cannyImage, (getStructuringElement(MORPH_RECT, Size(11, 11))));*/
		...
		findContours(differenceImage, contours, hierarchy, RETR_EXTERNAL, CHAIN_APPROX_SIMPLE);
		...
		for (int i = 0; i < contours.size(); i++) {
			...
			if (area > areaTH && area < areaMAX) {
				...
				else personas_cont = (w / anchura_pred);
				cout << w << "  para el contorno: " << personas_cont << endl;
				/*putText(Frame1, to_string(anchura_media), Point(20, 170), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0, 0, 255), 1, false);
				putText(Frame1, to_string(round(round(anchura_media) / w)), Point(20, 200), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0, 0, 255), 1, false);
				putText(Frame1, to_string(round(anchura_media) / w), Point(20, 230), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0, 0, 255), 1, false);*/
		...
		imshow("Frame2", differenceImage);
		writer.write(Frame1);
		writer2.write(differenceImage);
